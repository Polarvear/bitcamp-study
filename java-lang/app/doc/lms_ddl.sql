...
-- 강사
DROP TABLE IF EXISTS lms_tcher RESTRICT;
...
CREATE TABLE lms_lecture (
  ...
  sno      INTEGER      NULL     COMMENT '수강생번호', -- 수강생번호
  mgrno    INTEGER      NULL     COMMENT '회원번호', -- 회원번호
...
-- 강사
CREATE TABLE lms_tcher (
  tno    INTEGER NOT NULL COMMENT '강사번호', -- 강사번호
  wtna   INTEGER NOT NULL COMMENT '고용형태번호', -- 고용형태번호
  hr_pay INTEGER NULL     DEFAULT 0 COMMENT '시급' -- 시급
)
COMMENT '강사';

-- 강사
ALTER TABLE lms_tcher
  ADD CONSTRAINT PK_lms_tcher -- 강사 기본키
    PRIMARY KEY (
      tno -- 강사번호
    );
...
CREATE TABLE lms_stud (
  ...
  shc_nm  VARCHAR(30) NULL     COMMENT '최종학교', -- 최종학교
...
CREATE TABLE lms_mgr (
  mgrno   INTEGER     NOT NULL COMMENT '회원번호', -- 회원번호
  dno     INTEGER     NOT NULL COMMENT '부서번호', -- 부서번호
  pono    INTEGER     NOT NULL COMMENT '직금번호', -- 직금번호
...
ALTER TABLE lms_position
  MODIFY COLUMN pono INTEGER NOT NULL AUTO_INCREMENT COMMENT '직금번호';

-- 학력
CREATE TABLE lms_grade (
  gno   INTEGER     NOT NULL COMMENT '학력번호', -- 학력번호
  title VARCHAR(30) NULL     COMMENT '학력명' -- 학력명
)
COMMENT '학력';
...
ALTER TABLE lms_work_type
  MODIFY COLUMN wtna INTEGER NOT NULL AUTO_INCREMENT COMMENT '고용형태번호';
...
-- 강의
ALTER TABLE lms_lecture
  ADD CONSTRAINT FK_lms_stud_TO_lms_lecture -- 수강생 -> 강의
    FOREIGN KEY (
      sno -- 수강생번호
    )
...
-- 강사
ALTER TABLE lms_tcher
  ADD CONSTRAINT FK_lms_member_TO_lms_tcher -- 회원 -> 강사
    FOREIGN KEY (
      tno -- 강사번호
    )
...
-- 강사
ALTER TABLE lms_tcher
  ADD CONSTRAINT FK_lms_work_type_TO_lms_tcher -- 고용형태 -> 강사
    FOREIGN KEY (
      wtna -- 고용형태번호
    )
...
-- 강사배정
ALTER TABLE lms_lect_tchr
  ADD CONSTRAINT FK_lms_tcher_TO_lms_lect_tchr -- 강사 -> 강사배정
    FOREIGN KEY (
      tno -- 강사번호
    )
    REFERENCES lms_tcher ( -- 강사
      tno -- 강사번호
    );
