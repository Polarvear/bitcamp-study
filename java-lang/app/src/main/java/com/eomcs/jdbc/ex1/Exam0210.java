...
public class Exam0210 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    try {
      // JVM에서 jdbc driver 파일(.jar/META-INF/services/java.sql.Driver 파일)을 뒤져서 java.sql.Driver 를 구현한 클래스를 
      // 알아내어 자동으로 로딩하기 때문에,
      // 다음과 같이 명시적으로 로딩할 필요가 없다.
      // Class.forName("org.mariadb.jdbc.Driver");

      // DBMS에 연결하기
      //Driver 객체를 직접 사용하여 DBMS에 연결하지 않고 => 대신 DriverManager에게
      // DBMS와의 연결을 요청한다. 어느 서버에 접속할 것인지 정보를 제공하면
      // 그후 해당 DBMS 의 Driver 객체를 찾아 connect()를 호출한다.
      // jdbc url : DBMS 서버 정보. 
      //  예) jdbc:DBMS://서버주소:포트/데이터베이스명
      //      (포트번호를 지정하지 않으면 mysql 인 경우, 기본이 3306 이다.)
